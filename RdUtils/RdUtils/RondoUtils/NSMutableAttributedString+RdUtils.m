//
//  NSMutableAttributedString+RdUtils.m
...
#import "NSMutableAttributedString+RdUtils.h"
#import "RdUtilsManager.h"
#import "RdMacroFile.h"

@implementation NSMutableAttributedString (RdUtils)
...
+ (NSMutableAttributedString *_Nonnull)rd_AttributeString:(NSString *_Nonnull)string fontName:(NSString *_Nullable)name fontSize:(CGFloat)size strColor:(UIColor *_Nullable)color{
    ...
    if (name == nil) {
        if ([RdUtilsManager getManager].fontNameDefault.length != 0) {
            [attStr addAttribute:NSFontAttributeName value:[UIFont fontWithName:[RdUtilsManager getManager].fontNameDefault size:size] range:NSMakeRange(0, attStr.length)];
        }
        else {
